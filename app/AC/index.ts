...
import {
  ...
  CHANGE_SERVER_STATUS, CHANGE_CLIENT_PORT, CHANGE_SERVER_IP, CHANGE_SERVER_PORT,
} from "../constants";
...
export function changeServerStatus(status: string){
  return {
    payload: { status },
...
export function changeClientPort(port: number){
  return {
    payload: { port },
    type: CHANGE_CLIENT_PORT,
  }
}

export function changeServerIp(server_ip: string){
  return {
      payload: { server_ip },
      type: CHANGE_SERVER_IP,
  }
}

export function changeServerPort(server_port: string){
  return {
    payload: { server_port },
